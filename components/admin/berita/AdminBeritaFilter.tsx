...
import type { AdminKategoriFilter } from "@/lib/data/admin-berita";

interface AdminBeritaFilterProps {
  initialCari: string;
  activeKategori: AdminKategoriFilter;
}

const TABS: { key: AdminKategoriFilter; label: string }[] = [
...
export default function AdminBeritaFilter({ initialCari, activeKategori }: AdminBeritaFilterProps) {
  ...
  function handleTabClick(key: AdminKategoriFilter) {
    const params = new URLSearchParams(searchParams.toString());
    if (key === "semua") {
