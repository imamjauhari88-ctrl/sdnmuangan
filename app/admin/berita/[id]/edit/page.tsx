...
import { getAdminBeritaById } from "@/lib/data/admin-berita";
...
export default async function EditBeritaPage({ params }: EditBeritaPageProps) {
  ...
  const berita = await getAdminBeritaById(idNum);
  if (!berita) notFound();
  ...
  return (
    <div className="max-w-3xl mx-auto">
      <div className="mb-6">
        ...
        <h1 className="text-2xl font-bold text-gray-800 dark:text-white">Edit Berita</h1>
        <p className="text-sm text-gray-500 dark:text-gray-400 mt-1 line-clamp-1">{berita.judul}</p>
      </div>

      <BeritaForm mode="edit" initialData={berita} />
    </div>
  );
}
